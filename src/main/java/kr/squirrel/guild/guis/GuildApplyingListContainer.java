...
public class GuildApplyingListContainer extends Container {
    ...
    public GuildApplyingListContainer() {
        super(9 * 6, "[ 길드 목록 ]");
        setGUI();
    }
        ...
        switch (type) {
            case CLICK -> {
                ...
                if (CLICKED_ITEM != null && CLICKED_ITEM.getType() == Material.ENCHANTED_BOOK) {
                    ...
                    if (guild.getMemberCount() >= guild.getMaxPlayer()) {
                        Lang.send(player, Lang.ALREADY_FULL);
                        return;
                    }
                    if (guild.isFreeJoin()) {
                        guild.join(player);
                        player.closeInventory();
                        Lang.send(player, Lang.JOIN_GUILD);
                    } else {
                        if (guild.getApplyingPlayers().contains(player.getUniqueId().toString())) {
                            return;
                        }
                        guild.apply(player);
                        player.closeInventory();
                        Lang.send(player, Lang.APPLYING_COMPLETE);
                    }
                    return;
                }
                switch (CLICKED_SLOT) {
